const CONFIG = { DEMO_MODE: false, API_URL: "/api/generate-persona", AVATAR_MODE: "dicebear" };
const steps = [
  { id:'view-step-1', idx:'1/6', title:'Empresa/Projeto', help:'Qual o nome da empresa e o que ela faz?', hint:`<div class="helper"><p>Inclua:</p><ul><li>Nome da empresa/projeto</li><li>O que oferece (produtos/serviços)</li><li>Diferencial simples (ex.: “rápido”, “acolhedor”, “acessível”)</li></ul></div>`, field:'f-empresa', placeholder:'Digite aqui…' },
  { id:'view-step-2', idx:'2/6', title:'Público‑alvo', help:'Descreva o público que você quer transformar em persona.', hint:`<div class="helper"><p>Inclua alguns desses pontos:</p><ul><li>Idade aproximada</li><li>Ocupação</li><li>Rotina/estilo de vida</li></ul></div>`, field:'f-publico', placeholder:'Digite aqui…' },
  { id:'view-step-3', idx:'3/6', title:'Objetivos', help:'O que esse público deseja alcançar?', hint:`<div class="helper"><p>Pense no que a pessoa espera ganhar usando sua solução.</p></div>`, field:'f-objetivos', placeholder:'Digite aqui…' },
  { id:'view-step-4', idx:'4/6', title:'Desafios', help:'Quais são os principais desafios?', hint:`<div class="helper"><p>Barreiras comuns que atrapalham os objetivos.</p></div>`, field:'f-desafios', placeholder:'Digite aqui…' },
  { id:'view-step-5', idx:'5/6', title:'Motivações', help:'Por que esse público escolhe sua solução?', hint:`<div class="helper"><p>O que pesa na escolha.</p></div>`, field:'f-motivacoes', placeholder:'Digite aqui…' },
  { id:'view-step-6', idx:'6/6', title:'Comunicação', help:'Como esse público se comunica?', hint:`<div class="helper"><p>Onde falar e com qual tom.</p></div>`, field:'f-canais', placeholder:'Digite aqui…' },
];
...
steps.forEach((s,i)=>{
  ...
  sec.innerHTML = `<header class="stepbar"><span>Etapa ${s.idx}</span><h2>${s.title}</h2></header>
    <main class="content"><p class="help">${s.help}</p>${s.hint}<textarea id="${s.field}" placeholder="${s.placeholder}"></textarea></main>
    <footer class="actions"><button class="btn ghost" data-prev="${prev}">Voltar</button>${next ? `<button class="btn primary" data-next="${next}">Avançar</button>` : `<button class="btn primary" id="btn-gerar">Gerar persona 🤖</button>`}</footer>`;
...
setTimeout(()=> show('view-home'), 1500);
...
document.getElementById('modal-conceito').addEventListener('click', (e)=>{
  if (e.target.dataset.close === 'modal' || e.target.closest('[data-close="modal"]')){
    document.getElementById('modal-conceito').classList.add('hidden');
  }
});
async function gerarPersonas(payload){
  const r = await fetch("/api/generate-persona", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
function setAvatar(nome){ const img=document.getElementById('p-avatar'); const n=(nome||'P').trim()||'P'; img.src='https://api.dicebear.com/8.x/bottts/svg?seed='+encodeURIComponent(n); }
...
document.getElementById('btn-exportar').addEventListener('click', ()=>{ let menu=document.querySelector('.export-menu'); if(menu) menu.remove(); menu=document.createElement('div'); menu.className='card persona export-menu'; menu.innerHTML=`<b>Exportar</b><br><button class="btn" id="exp-png">Imagem (PNG)</button><button class="btn" id="exp-pdf">PDF (A4)</button><button class="btn ghost" id="exp-close">Fechar</button>`; document.body.appendChild(menu); document.getElementById('exp-close').onclick=()=>menu.remove(); document.getElementById('exp-png').onclick=exportPNG; document.getElementById('exp-pdf').onclick=exportPDF; });
